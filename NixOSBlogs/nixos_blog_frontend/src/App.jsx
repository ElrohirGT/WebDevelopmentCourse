...
export default function App() {
	...
	useEffect(() => {
		...
		if (currentUrlRoute === "/admin" || currentUrlRoute === "/admin/") {
			navigateToAdminView();
		}
	}, []);
	...
	routeToComponentMapper[ROUTES.blogDetails] = (
		<BlogDetailsView
			blogPreview={currentBlogPreview}
			navigateToMainView={navigateToMainView}
			loginToken={loginToken}
			navigateToUpdateBlogForm={navigateToUpdateBlogForm}
		/>
	);
	routeToComponentMapper[ROUTES.login] = <LoginView onLogin={onLogin} />;
	routeToComponentMapper[ROUTES.admin] = (
		<AdminView
			blogsPreviewsResource={blogsPreviewsResource}
			loginToken={loginToken}
			navigateToBlogDetails={navigateToBlogDetails}
			navigateToCreateBlogForm={navigateToCreateBlogForm}
			deleteBlog={(id) => deleteBlog(loginToken, id)}
		/>
	);
	routeToComponentMapper[ROUTES.createBlogForm] = <CreateBlogForm onSubmit={onCreateBlogFormSubmit} />;
	routeToComponentMapper[ROUTES.updateBlogForm] = (
		<UpdateBlogForm onSubmit={onUpdateBlogFormSubmit} blog={currentBlogData} />
	);
