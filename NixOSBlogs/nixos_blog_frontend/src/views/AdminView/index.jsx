...
export default function AdminView(
  { loginToken, blogsPreviewsResource, navigateToBlogDetails, navigateToCreateBlogForm, deleteBlog },
) {
  ...
  return (
    <div className="adminViewContainer">
      ...
      <Suspense fallback={<p>Loading...</p>}>
        <AdminBlogList
          blogResource={blogsPreviewsResource}
...
function AdminBlogList({ blogResource, navigateToBlogDetails, deleteBlog }) {
  ...
  return (
    <div className="adminBlogsPreview">
      {error.length > 0 ? <p className="ErrorText ErrorInAdmin">{error}</p> : null}
      {blogs.map(b => {
        return (
          <div className="adminBlogListItem" key={b.id} onClick={() => navigateToBlogDetails(b)}>
            <h3>{b.title}</h3>
            <div className="adminBlogListItemControls">
              <button className="PrimaryButton NegativeButton" type="button" onClick={(ev) => onDelete(ev, b.id)}>
                Delete
              </button>
            </div>
          </div>
